...
fn set_musicbrainz_id(path: &Path, release_id: &str, relative_path: &str, tx: &mpsc::Sender<String>) -> Result<()> {
    match lofty::read_from_path(path) {
        Ok(tagged_file) => {
            ...
            if let Some(tag) = tags.first() {
                ...
                if let Some(existing_id) = tag.get_string(&lofty::tag::ItemKey::MusicBrainzReleaseId) {
            ...
            tx.send(format!("COMPLETED: {} - Skipped (format not supported by lofty)", relative_path))?;
        },
        Err(_) => {
            // Skip files that lofty can't handle at all
            tx.send(format!("COMPLETED: {} - Skipped (unsupported format)", relative_path))?;
        }
    }
...
fn get_artist_album_from_path(path: &Path, folder_artist: &str, folder_album: &str) -> (String, String) {
    match lofty::read_from_path(path) {
        Ok(tagged_file) => {
            // Use the TaggedFileExt trait to access tags
            use lofty::file::TaggedFileExt;
            ...
            if let Some(tag) = tags.first() {
                ...
                let artist = tag.get_string(&lofty::tag::ItemKey::TrackArtist).unwrap_or(folder_artist);
                let album = tag.get_string(&lofty::tag::ItemKey::AlbumTitle).unwrap_or(folder_album);
...
pub async fn process_single_album_sync_tags(album_path: &Path, tx: mpsc::Sender<String>) -> Result<()> {
    ...
    let folder_artist = artist_path.file_name().unwrap().to_str().unwrap().to_string();
    let folder_album = album_path.file_name().unwrap().to_str().unwrap().to_string();
    ...
    let all_extensions: Vec<_> = ID3_EXTENSIONS.iter()
    ...
    tx.send(format!("COMPLETED: Scanned {} files ({} audio files found, {} skipped)",
                   files_scanned, audio_files.len(), files_skipped))
    ...
    let album_groups: HashMap<(String, String), Vec<PathBuf>> = audio_files
        ...
        .fold(
            ...
            |mut groups: HashMap<(String, String), Vec<PathBuf>>, path| {
                let (artist, album) = get_artist_album_from_path(&path, &folder_artist, &folder_album);
    ...
    tx.send(format!("COMPLETED: Grouped {} audio files into {} album groups",
                   audio_files_count, album_groups.len()))
    ...
    for ((artist, album), _) in &album_groups {
        if !release_cache.contains_key(&(artist.clone(), album.clone())) {
                ...
                .await
            {
                Ok(search_result) => {
                    ...
                    tx.send(format!("COMPLETED: MusicBrainz search for {} - {}", artist, album))
                        .context("Failed to send MusicBrainz progress")?;
                },
                Err(e) => {
                    eprintln!("MusicBrainz search failed for {} - {}: {}", artist, album, e);
                    release_cache.insert((artist.clone(), album.clone()), None);
                    // Still count as completed task even if failed
                    tx.send(format!("COMPLETED: MusicBrainz search for {} - {} (failed)", artist, album))
                        .context("Failed to send MusicBrainz progress")?;
                }
            }
        }
    ...
    for ((artist, album), paths) in album_groups.into_iter() {
        ...
        if let Some(Some(release_id)) = release_cache.get(&(artist.to_string(), album.to_string())) {
            ...
            paths.into_par_iter().for_each_with(tx.clone(), |tx, path| {
                let result = (|| -> Result<()> {
                    let relative_path = path.strip_prefix(&album_path)
                        .unwrap_or(&path)
                        .display()
                        .to_string();
            ...
            tx.send(format!("COMPLETED: Finished processing {} - {} ({} files processed)",
                           artist, album, paths_len))
                .context("Failed to send album summary")?;
        } else {
            tx.send(format!("COMPLETED: Skipped {} - {} (no MusicBrainz match found)",
                          artist, album))
                .context("Failed to send no match message")?;
        }
    }

    tx.send(format!("Successfully synchronized all files in {}", folder_album))
