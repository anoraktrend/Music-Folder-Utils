...
use discid::{DiscId};
use musicbrainz_rs::{entity::release::Release, prelude::*, MusicBrainzClient, ApiRequest};
use std::path::Path;
use std::sync::mpsc;
use std::fs;
...
pub async fn import_cd(
    ...
    let cd_info = read_cd_from_device(device, tx.clone()).await?;
    ...
    let cd_info = lookup_cd_info(&cd_info, tx.clone()).await?;
    ...
    for (i, track) in cd_info.tracks.iter().enumerate() {
        ...
        match tokio::time::timeout(
            ...
            import_cd_track(device, &cd_info, track, &album_dir, tx.clone(), cover_art_data.as_ref())
        ).await {
            Ok(Ok(())) => {
...
async fn read_cd_from_device(device: &str, tx: mpsc::Sender<String>) -> Result<CdInfo> {
